// Phone validation - valid US area codes + fake pattern detection
...
export function validatePhone(phoneStr: string): { valid: boolean; msg?: string } {
  let digits = phoneStr.replace(/\D/g, "");
  if (digits.startsWith("1")) digits = digits.slice(1);
  if (digits.length !== 10) return { valid: false, msg: "Please enter a valid 10-digit US phone number." };
  const area = digits.slice(0, 3);
  if (!VALID_AREA_CODES.has(area)) return { valid: false, msg: `That area code (${area}) doesn't appear to be valid. Please check your number.` };
...
// Email validation - disposable domain blocking + fake pattern detection
...
const FAKE_PATTERNS = ["test@test", "fake@fake", "asdf@asdf", "noemail@", "spam@", "junk@", "nobody@nobody", "aaa@aaa", "abc@abc", "example@example"];

export function validateEmail(email: string): { valid: boolean; msg?: string } {
  if (!email || email.trim() === "") return { valid: true };
  const cleaned = email.trim().toLowerCase();
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(cleaned)) return { valid: false, msg: "Please enter a valid email address." };
  ...
  for (const pattern of FAKE_PATTERNS) {
    if (cleaned.startsWith(pattern)) return { valid: false, msg: "Please enter your real email address." };
  }
  return { valid: true };
}
